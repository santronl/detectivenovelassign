...
import { User, Heart, ArrowDown, ArrowUp, Maximize, Minimize, GitBranch, X, UserMinus, MousePointer2, UserRoundPlus, ChevronLeft, ChevronRight, Hash, GripHorizontal, Crosshair, Link2, Edit3, HelpCircle, MousePointer, MoveHorizontal, Search } from 'lucide-react';
...
const FamilyTree: React.FC<Props> = ({ 
    ...
    return (
        <div 
            className={`flex flex-row transition-all overflow-hidden border border-slate-800 rounded-3xl
                ${isExpanded ? 'fixed inset-0 z-[1000] bg-slate-950 !rounded-none !border-none' : 'relative h-full min-h-[650px] bg-slate-900/20'}`}
        >
            {/* Expanded Sidebar */}
            {isExpanded && (
                <div className="w-72 border-r border-slate-800 bg-slate-900 flex flex-col shrink-0 animate-in slide-in-from-left duration-300">
                    <div className="p-4 border-b border-slate-800 bg-slate-950/50">
                        <h3 className="text-sm font-black text-white uppercase tracking-widest flex items-center gap-2 mb-3">
                            <User size={16} className="text-indigo-400"/> 登场人物
                        </h3>
                        <div className="relative">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={14} />
                            <input 
                                className="w-full bg-slate-800 border border-slate-700 rounded-xl pl-9 pr-3 py-2 text-xs text-white outline-none focus:ring-1 focus:ring-indigo-500"
                                placeholder="搜索人物拖入..."
                                value={searchTerm}
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
                        {characters
                            .filter(c => !c.isVirtual && (searchTerm === "" || c.name.toLowerCase().includes(searchTerm.toLowerCase())))
                            .map(char => {
                            ...
                            return (
                                <div 
                                    ...
                                    className={`flex items-center gap-3 p-2.5 rounded-xl border transition-all ${isActive ? 'bg-slate-900/50 border-slate-800 opacity-50 grayscale cursor-default' : 'bg-slate-800 border-slate-700 hover:border-indigo-500 hover:bg-slate-750 cursor-grab active:cursor-grabbing hover:shadow-md'}`}
                                >
                                    <div className="w-8 h-8 rounded-full bg-slate-900 border border-slate-600 flex items-center justify-center overflow-hidden shrink-0 shadow-sm">
                                    ...
                                    {isActive && <div className="ml-auto"><div className="w-1.5 h-1.5 bg-green-500 rounded-full shadow-[0_0_8px_rgba(34,197,94,0.6)]" /></div>}
                                </div>
                            );
                        })}
                        <div className="pt-4 px-2 pb-2">
                            <button onClick={() => setIsAddingVirtual(true)} className="w-full py-2 border border-dashed border-slate-700 rounded-xl text-[10px] font-bold text-slate-500 hover:text-indigo-400 hover:border-indigo-500/50 transition-all flex items-center justify-center gap-2">
                                <UserRoundPlus size={12} /> 新增虚拟节点
                            </button>
                        </div>
                    </div>
                </div>
            ...
            >
                ...
                <div className="absolute top-6 right-6 z-20 flex gap-2">
                    ...
                    <button onClick={() => setIsExpanded(!isExpanded)} className={`p-2.5 bg-slate-800 hover:bg-slate-700 text-slate-400 rounded-xl border border-slate-700 shadow-xl transition-all active:scale-95 ${isExpanded ? 'bg-slate-700 text-white' : ''}`}>
