...
import { Alibi, Character, TimePoint, Location } from '../types';
import { CheckCircle2, HelpCircle, XCircle, Plus, Trash2, Edit3, X, Users, Clock, MapPin, AlignLeft, ShieldCheck, Check, Calendar, ChevronDown } from 'lucide-react';
...
const AlibiMatrix: React.FC<Props> = ({ alibis, characters, timePoints, locations, onAddAlibi, onUpdateAlibi, onDeleteAlibi }) => {
  ...
  const handleOpenEdit = (alibi: Alibi, index: number) => {
    ...
    setLocationText(alibi.location);
    setSelectedLocationId(alibi.locationId);
  ...
  const handleSubmit = () => {
    if (selectedCharIds.length === 0 || (!locationText && !selectedLocationId)) return;

    // 如果选了地点库里的地点，优先同步名称
    const finalLocationName = selectedLocationId 
      ? locations.find(l => l.id === selectedLocationId)?.name || locationText 
      : locationText;
    ...
    <div className="space-y-6">
        ...
        <table className="w-full text-left text-sm text-slate-300">
          <thead className="bg-slate-900/80 text-slate-400 uppercase font-bold text-[11px] tracking-wider">
            <tr>
              ...
              <th className="px-6 py-4">时间段 / 轨迹点</th>
                ...
                <tr key={idx} className="hover:bg-slate-700/30 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                       <MapPin size={12} className={alibi.locationId ? "text-emerald-500" : "text-slate-500"} />
                       <span className={alibi.locationId ? "text-emerald-400 font-bold" : ""}>{alibi.location}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleOpenEdit(alibi, idx)} className="p-2 text-slate-400 hover:text-blue-400 hover:bg-slate-700 rounded-lg transition-all"><Edit3 size={16} /></button>
                    <button onClick={() => onDeleteAlibi(idx)} className="p-2 text-slate-400 hover:text-red-400 hover:bg-slate-700 rounded-lg transition-all"><Trash2 size={16} /></button>
                  </td>
                </tr>
              ))
            )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-slate-800 rounded-2xl border border-slate-600 shadow-2xl w-full max-w-md animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar">
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2"><Users size={14} /> 涉及角色 <span className="text-red-500">*</span></label>
                <div className="bg-slate-900 border border-slate-700 rounded-xl overflow-hidden p-2 grid grid-cols-2 gap-1 max-h-32 overflow-y-auto custom-scrollbar">
                  {characters.map(c => (
                    <button
                      ...
                      className={`flex items-center justify-between px-3 py-1.5 rounded-lg text-xs transition-all ${selectedCharIds.includes(c.id) ? 'bg-purple-900/30 text-purple-200 border border-purple-500/30' : 'text-slate-500 hover:bg-slate-800 border border-transparent'}`}
                    >
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2"><Clock size={14} /> 关联空间轨迹时间点</label>
                  <div className="flex flex-wrap gap-2">
                    {timePoints.map(tp => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${selectedTimePointId === tp.id ? 'bg-purple-600 border-purple-400 text-white shadow-lg' : 'bg-slate-900 border-slate-700 text-slate-500 hover:border-slate-500'}`}
                      >
                        {tp.label}
                      </button>
                    ))}
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2"><Calendar size={14} /> 自定义时间段 <span className="text-red-500">*</span></label>
                  <input 
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2"><MapPin size={14} /> 选择地点索引 <span className="text-red-500">*</span></label>
                  <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto custom-scrollbar p-1">
                    {locations.map(loc => (
                      <button
                        key={loc.id}
                        onClick={() => setSelectedLocationId(loc.id)}
                        className={`px-3 py-2 rounded-xl text-[10px] font-bold border transition-all truncate text-left ${selectedLocationId === loc.id ? 'bg-emerald-600 border-emerald-400 text-white shadow-lg' : 'bg-slate-900 border-slate-700 text-slate-500 hover:border-slate-500'}`}
                      >
                        {loc.name}
                      </button>
                    ))}
                  ...
                  <div className="pt-2">
                    <input 
                      ...
                      onChange={(e) => {
                        ...
                        if(selectedLocationId) setSelectedLocationId(undefined);
                      }}
                      placeholder="或手动录入临时地点..."
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2"><ShieldCheck size={14} /> 状态验证</label>
                <div className="grid grid-cols-3 gap-2">
                  {(['确凿', '模糊', '无证明'] as Alibi['status'][]).map(s => (
                    <button key={s} onClick={() => setStatus(s)} className={`py-2 rounded-xl text-xs font-bold border transition-all ${status === s ? 'bg-purple-600 border-purple-400 text-white shadow-lg' : 'bg-slate-900 border-slate-700 text-slate-500 hover:border-slate-500'}`}>{s}</button>
                  ))}
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2"><AlignLeft size={14} /> 证言详情</label>
                <textarea className="w-full h-24 bg-slate-900 border border-slate-700 rounded-xl p-3 text-white text-xs focus:ring-2 focus:ring-purple-500 outline-none resize-none transition-all" value={details} onChange={(e) => setDetails(e.target.value)} placeholder="详细描述活动与人证..." />
              </div>
            </div>
            ...
            <div className="p-4 border-t border-slate-700 flex justify-end gap-3 bg-slate-900/20">
              ...
              <button onClick={handleSubmit} disabled={selectedCharIds.length === 0 || (!locationText && !selectedLocationId)} className="px-8 py-2.5 bg-purple-600 hover:bg-purple-500 disabled:opacity-40 text-white rounded-xl text-sm font-bold shadow-lg">确认录入</button>
            </div>
          </div>
        </div>
      )}
